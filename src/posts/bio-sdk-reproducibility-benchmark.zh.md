...
最后说点实在话：这批数据我自己看着都嫌抽象。r=1.000000、SHA256 完全
一致——别人听着可能没什么感觉，但它的意思其实很直白：同一个工具、同一套
参数、同一份数据，再跑一遍就该得到一模一样的结果。我们做到了；而且参数
只要错一个，立刻就能看出来（对照组 r=0.981 掉下去了）。数据就在这儿，
是多少就是多少，我们原样放出来。比起让大家记住哪个数字，我更想说明白的
是：这套东西的结果能被严格验证，出错会直接报错而不是悄悄出数，所有边界
都摆在明面上。
